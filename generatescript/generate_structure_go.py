...
GENERATE_FILE = os.path.join(os.path.dirname(__file__), "ctpwrapper/ApiStructure_go.py")
...
def generate_struct(struct, struct_doc, py_file):
    for item in struct:
        ...
        py_file.write("type {class_name} struct {{\n".format(class_name=item))
        ...
        for field in struct_dict:
            ...
            py_file.write("        {field} {fieldtype}\n        // {doc}\n".format(field=field,fieldtype=field_data["type"], doc=field_doc))
